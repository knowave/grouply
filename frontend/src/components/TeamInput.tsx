...
export function TeamInput({ teams, onAdd, onUpdate, onRemove }: TeamInputProps) {
  return (
    <section className="rounded-md border border-black/10 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-lg font-bold">{MESSAGES.teamsTitle}</h2>
        <button
        ...
        {teams.map((team, index) => (
          <div key={index} className="flex gap-2">
            <input
              ...
              value={team}
              placeholder={MESSAGES.teamPlaceholder}
